...
def run_merchant_middleware(
    ...
    for t in (1, 2, 3):
        tier_groups[t].sort(key=lambda c: c.merchant.price_usd)
    ...
    for t in (1, 2, 3):
        ...
        for c in items:
            log.info("    %-35s  $%.2f  rank=%.2f  region=%s",
                     c.merchant.name, c.merchant.price_usd, c.score, c.region)
    ...
    if tier_groups[1] and tier_groups[3] and not show_global_tier:
        cheapest_t1 = min(c.merchant.price_usd for c in tier_groups[1])
        cheapest_t3 = min(c.merchant.price_usd for c in tier_groups[3])
    ...
    for i, c in enumerate(active_results, 1):
        ...
        log.info("  %d. T%d %-35s  rank=%-7.2f  $%.2f  %.2f km  %s",
                 ...
                 c.merchant.price_usd, c.distance_km, badge)
